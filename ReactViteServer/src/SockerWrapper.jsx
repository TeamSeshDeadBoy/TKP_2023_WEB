        ...
        if (choiceInd == quiz.questions[questInd].validIndex) {
          if (scores.find(obj => obj.userId == id)){
            if (scores.find(obj => obj.userId ===id && obj.answers.indexOf(questInd) == -1)) {
              console.log("Coins of user:",scores.filter(obj => obj.userId == id))
              scores = [...scores.filter(user => user.userId != id), {userId: id, coins: scores.filter(obj => obj.userId == id)[0].scores + 50, answers: [...  scores.find(aa => aa.userId == id).answers, questInd]}]
